...
CONFIG: Dict = {
    ...
    # NFTM-style masked training (hybrid)
    "use_masked_loss": True,
    "mask_mode": "hybrid",               # static | dynamic | hybrid
    "mask_alpha_threshold": 0.05,
    "mask_color_threshold": 0.05,        # non-white detection threshold
    "mask_dilation": 4,                  # buffer pixels around active region
...
def build_static_target_mask(target: torch.Tensor, cfg: Dict) -> torch.Tensor:
    """
    Static region from target image: non-white (RGBA premultiplied).
    """
    with torch.no_grad():
        alpha_thr = cfg["mask_alpha_threshold"]
        color_thr = cfg["mask_color_threshold"]
        alpha = target[3:4]
        rgb = target[:3]
        # White canvas => rgb ~ 1.0
        color_diff = torch.mean(torch.abs(rgb - 1.0), dim=0, keepdim=True)
        mask = (alpha > alpha_thr) & (color_diff > color_thr)
        mask = mask.float().unsqueeze(0)  # [1,1,H,W]
        mask = dilate_mask(mask, cfg["mask_dilation"])
        return (mask > 0.0).float()

def build_dynamic_mask(x: torch.Tensor, cfg: Dict) -> torch.Tensor:
    with torch.no_grad():
        alpha_thr = cfg["mask_alpha_threshold"]
        alive = F.max_pool2d(x[:, 3:4], 3, 1, 1) > alpha_thr
        mask = alive.float()
        mask = dilate_mask(mask, cfg["mask_dilation"])
        return (mask > 0.0).float()

def build_hybrid_mask(x: torch.Tensor, static_mask: torch.Tensor, cfg: Dict) -> torch.Tensor:
    with torch.no_grad():
        dynamic = build_dynamic_mask(x, cfg)
        static = static_mask.expand(x.shape[0], -1, -1, -1)
        return torch.clamp(static + dynamic, 0, 1)

def masked_mse_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """
    pred/target: [B,4,H,W], mask: [B,1,H,W]
    """
    diff = (pred - target) ** 2
    mask = mask.expand(-1, diff.shape[1], -1, -1)
    num = (diff * mask).sum()
    den = mask.sum().clamp_min(eps) * diff.shape[1]
    return num / den
...
def train_model(model_name, target_np, cfg):
    ...
    p = cfg["padding"]
    pad_target = np.pad(target_np, [(p,p), (p,p), (0,0)], mode='constant', constant_values=1.0)
    target = torch.from_numpy(pad_target).permute(2,0,1).to(device)
    size = target.shape[1]

    # Static mask built from target
    static_mask = build_static_target_mask(target, cfg).to(device)
            ...
            try:
                ...
                with torch.amp.autocast('cuda', enabled=cfg["mixed_precision"]):
                    ...
                    if cfg["use_masked_loss"]:
                        if cfg["mask_mode"] == "static":
                            mask = static_mask.expand(x.shape[0], -1, -1, -1)
                        elif cfg["mask_mode"] == "dynamic":
                            mask = build_dynamic_mask(x, cfg)
                        else:
                            mask = build_hybrid_mask(x, static_mask, cfg)
                        pred = to_rgba(x)
                        tgt = target.unsqueeze(0).expand(pred.shape[0], -1, -1, -1)
                        mse = masked_mse_loss(pred, tgt, mask)
                    else:
